...
public final class MathUtils {
	...
	static public int random(int range) {
		return RandomThreadLocal.get().nextInt(range + 1);
	}

	/** Returns a random number between start (inclusive) and end (inclusive). */
	static public int random(int start, int end) {
		return start + RandomThreadLocal.get().nextInt(end - start + 1);
	}
	...
	static public long random(long range) {
		return (long) (RandomThreadLocal.get().nextDouble() * range);
	}

	/** Returns a random number between start (inclusive) and end (inclusive). */
	static public long random(long start, long end) {
		return start + (long) (RandomThreadLocal.get().nextDouble() * (end - start));
	}
	...
	static public float random(float range) {
		return RandomThreadLocal.get().nextFloat() * range;
	}

	/** Returns a random number between start (inclusive) and end (exclusive). */
	static public float random(float start, float end) {
		return start + RandomThreadLocal.get().nextFloat() * (end - start);
	}

	/** Returns -1 or 1, randomly. */
	static public int randomSign() {
		return 1 | (RandomThreadLocal.get().nextInt() >> 31);
	}
	...
	public static float randomTriangular() {
		return RandomThreadLocal.get().nextFloat() - RandomThreadLocal.get().nextFloat();
	}
	...
	public static float randomTriangular(float max) {
		return (RandomThreadLocal.get().nextFloat() - RandomThreadLocal.get().nextFloat()) * max;
	}
	...
	public static float randomTriangular(float min, float max, float mode) {
		final float u = RandomThreadLocal.get().nextFloat();
	...
	public static float lerpAngle(float fromRadians, float toRadians, float progress) {
		final float delta = ((toRadians - fromRadians + PI2 + PI) % PI2) - PI;
	...
	public static float lerpAngleDeg(float fromDegrees, float toDegrees, float progress) {
		final float delta = ((toDegrees - fromDegrees + 360 + 180) % 360) - 180;
	 ...
	 *            the second value.
	 * @param tolerance
	 *            represent an upper bound below which the two values are considered
	 *            equal.
	 */
