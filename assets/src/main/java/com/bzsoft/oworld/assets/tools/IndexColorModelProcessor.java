...
public final class IndexColorModelProcessor {

	protected static class Cube { // structure for a cube in color space
	...
	private final int[] hist; // RGB histogram and reverse
								// color lookup table
	...
	private IndexColorModelProcessor(int[] pixels, int width, int height) {
		int color16;
		pixels32 = pixels;
		this.width = width;
		this.height = height;
	...
	public IndexColorModelProcessor(BufferedImage img) {
		this(img.getRGB(0, 0, img.getWidth(), img.getHeight(), null, 0, img.getWidth()), img.getWidth(),
				img.getHeight());
	}

	// Convert from 24-bit to 15-bit color
	private final int rgb(int c) {
		final int r = (c & 0xf80000) >> 19;
		final int g = (c & 0xf800) >> 6;
		final int b = (c & 0xf8) << 7;
		return b | g | r;
	}
	 ...
	 * "hist" into "maxcubes" cubes. The centroids (average value) of each cube are
	 * are used to create a color table. "hist" is then updated to function as an
	 * inverse color map that is used to generate an 8-bit image.
	 */
	/*
	 * public Image convert(int maxcubes) { ImageProcessor ip =
	 * convertToByte(maxcubes); return ip.createImage(); }
	 */
	...
	public BufferedImage convertToByte(int maxcubes) {
		...
		while (ncubes < maxcubes) {
			...
			// Search the list of cubes for next cube to split, the lowest level
			// cube
			level = 255;
			splitpos = -1;
	...
	protected void makeInverseMap(int[] hist, int ncubes) {
		...
		gLUT[ncubes] = (byte) 0;
		bLUT[ncubes] = (byte) 0;
		cm = new IndexColorModel(8, ncubes + 1, rLUT, gLUT, bLUT, ncubes);
	...
	protected void quickSort(int a[], int lo0, int hi0) {
		// Based on the QuickSort method by James Gosling from Sun's SortDemo
		// applet
	...
	protected BufferedImage makeImage(int ncubes) {
		...
		for (int i = 0; i < width * height; i++) {
			pixel = pixels32[i];
			if (pixel == 0) {
				pixels8[i] = (byte) ncubes;
			} else {
				color16 = rgb(pixel);
				pixels8[i] = (byte) hist[color16];
			}
		}
		// BufferedImage ip = new BufferedImage(width, height, pixels8, cm);
		final BufferedImage bimg = new BufferedImage(width, height, BufferedImage.TYPE_BYTE_INDEXED, cm);
		final DataBuffer db = new DataBufferByte(pixels8, pixels8.length);
		final Raster r = Raster.createPackedRaster(db, width, height, 8, null);
		bimg.setData(r);
		return bimg;
	}
